...
| `qwen27b/w4a16/fast/dflash2-tqk8v4-1x256k-text-image.env` | fast | 256K x1 | TQK8V4 | DFlash2/7 | text+image | 318,010 | not measured |
...
JIT rejection and TurboQuant kernels. The text-image 256K route has been
validated for startup, multimodal health, and capacity, but does not have a
comparable image-throughput measurement yet, so no synthetic text-only number
is attributed to it.
