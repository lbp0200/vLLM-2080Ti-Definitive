...
的 PCIe CAR 修复（`a910fe9e43`）。它们依赖 ABI 匹配的扩展，未打该补丁的 pre4
不是等价路线。四组运行均未禁用 custom all-reduce，日志均为
`['CUSTOM', 'PYNCCL']`，通过启动和图像请求；性能取三个不同 prompt 的 4K/128
独立请求中位数。fast TQK8V4 路线在正式测量前单独执行过一次 4K/128 warmup，并在
SM75 上使用 FlashInfer FA2。fast FP16 MTP3 候选未列在下表：其 full-graph 质量探针
失败（GSM8K 为 0/32，normal FP16 为 21/32，且检索、多轮和图像 OCR 均失败），必须保持
实验状态。图像 smoke 仅验证视觉请求路由和生成，不代表视觉回答质量已完成评估。
