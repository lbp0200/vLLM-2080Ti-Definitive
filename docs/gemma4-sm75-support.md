...
`0.2.1-pre4` README. The README's statement of baseline Gemma4 support means
...
`0.2.1-pre4` is based on vLLM nightly `b23433088b` (`v0.29.1rc0-33`), CUDA 13.0, and PyTorch 2.13. No Gemma4
...
| Model form | Intended use | SM75 `0.2.1-pre4` status |
| --- | --- | --- |
...
| Gemma4 assistant model types | MTP/speculative assistant checkpoints | Requires the Gemma4 MTP path; no `0.2.1-pre4` SM75 promotion evidence |
...
hardware-specific validation table.
...
`0.2.1-pre4` tested-checkpoint list.

| Target checkpoint | Weight route | Historical route state | `0.2.1-pre4` interpretation |
| --- | --- | --- | --- |
...
The current MTP implementation and assistant model support must be checked in
the matching vLLM source and in the validation report before promotion.
...
`0.2.1-pre4` text-plus-image, video, or audio deployment preset. Multimodal
