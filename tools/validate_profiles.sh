...
expected_profiles=(
  2x2080Ti/qwen27b/w8a16/nomtp-fp16kv-1x119k-text-image.env
  2x2080Ti/qwen27b/w8a16/nomtp-fp16kv-1x172k-text-only.env
  2x2080Ti/qwen27b/w8a16/mtp4-fp16kv-1x145k-text-only.env
  2x2080Ti/qwen27b/w8a16/mtp4-fp8kv-1x182k-text-image.env
  2x2080Ti/qwen27b/w8a16/mtp4-fp8kv-1x256k-text-only.env
  2x2080Ti/qwen27b/w8a16/yarn-fp8kv-1x331k-text-only.env
  2x2080Ti/qwen27b/w4a16/dflash2-fp8kv-1x256k-text-image.env
  2x2080Ti/qwen27b/w4a16/yarn-fp8kv-1x512k-text-only.env
  2x2080Ti/qwen27b/w4a16/mtp4-fp8kv-2x224k-text-only.env
  2x2080Ti/qwen27b/w4a16/dflash-fp8kv-2x172k-text-only.env
  2x2080Ti/qwen27b/w4a16/mtp4-tq4nc-3x256k-text-only.env
  2x2080Ti/qwen35b/w8a16/nomtp-fp16kv-1x256k-text-only.env
  2x2080Ti/qwen35b/w8a16/nomtp-fp8kv-1x216k-text-image.env
  4xT10/qwen27b/w8a16/mtp4-fp16kv-1x256k-text-image.env
  4xT10/qwen27b/w8a16/dflash2-fp16kv-1x256k-text-only.env
  4xT10/qwen27b/w8a16/dflash2-fp16kv-1x235k-text-image.env
  4xT10/qwen27b/w8a16/mtp3-fp8kv-2x256k-text-image.env
)
...
while IFS= read -r -d '' file; do
  ...
  case "$kv" in
    float16|fp8|turboquant_4bit_nc) ;;
    auto|default|"")
  ...
  profile_name=${rel##*/}
  if [[ "$profile_name" =~ mtp([0-9]+)- ]] && [[ "$speculative_tokens" != "${BASH_REMATCH[1]}" ]]; then
  ...
  if [[ "$profile_name" =~ -([1-9][0-9]*)x[0-9]+k- ]]; then
  ...
  if [[ "$profile_name" =~ -[1-9][0-9]*x([1-9][0-9]*)k- ]]; then
    expected_context=$((10#${BASH_REMATCH[1]} * 1024))
    actual_context=$(read_profile_value "$file" MAX_MODEL_LEN)
    # Context labels are nominal (e.g. 235K); allow the final block-aligned
    # value to differ by at most one 1K block after capacity probing.
    context_delta=$((actual_context - expected_context))
    (( context_delta < 0 )) && context_delta=$((-context_delta))
    (( context_delta <= 1024 )) || \
