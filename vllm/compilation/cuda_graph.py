...
def _copy_tensor_tree(src: Any, dst: Any) -> None:
    ...
    if dataclasses.is_dataclass(src) and not isinstance(src, type):
        src = {
            field.name: getattr(src, field.name) for field in dataclasses.fields(src)
        }
...
class CUDAGraphWrapper:
    ...
    Note: CUDAGraphWrapper does not own persistent model input buffers. Their
    management remains outside this wrapper because it makes no assumptions
    about dynamic input shapes. FULL graphs do retain forward-context tensor
    references and refresh their contents before replay. Input address checking
    is enabled when VLLM_LOGGING_LEVEL == "DEBUG".
    """
