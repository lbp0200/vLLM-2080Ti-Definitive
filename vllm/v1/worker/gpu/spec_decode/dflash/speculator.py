...
class DFlashSpeculator(DraftModelSpeculator):
    ...
    def propose(
        ...
        if batch_desc.cg_mode == CUDAGraphMode.FULL:
            assert self.query_cudagraph_manager is not None
            self.query_cudagraph_manager.run_fullgraph(
                batch_desc,
                attn_metadata=draft_attn_metadata,
                slot_mapping=draft_slot_mappings_by_layer,
            )
        else:
            self._generate_draft(
                num_reqs,
...
def _prepare_dflash_inputs_kernel(
    ...
    if block_idx == 0:
        ...
        # tokens this step, minus cache-restored whole kernel blocks. Those
        # slots hold no draft KV and are shifted out of the block table below.
        num_cached = tl.load(num_cached_tokens_ptr + req_state_idx)
        cached_block_span = block_size * CP_SIZE
        num_shifted_slots = (num_cached // cached_block_span) * cached_block_span
        draft_seq_len = tl.maximum(
            last_valid_pos + 1 + num_query_per_req - num_shifted_slots,
            num_query_per_req,
        )
        tl.store(
            out_seq_lens_ptr + req_idx,
            tl.minimum(draft_seq_len, max_model_len),
        )
        # Copy sampling state.
        tl.store(
...
def _shift_draft_block_tables_kernel(
    ...
    req_state_idx = tl.load(idx_mapping_ptr + req_idx)
    num_cached = tl.load(num_cached_tokens_ptr + req_state_idx)
    block_span = block_size * CP_SIZE
    shift = num_cached // block_span
    if shift == 0:
        return
    ...
    num_needed = (seq_len + block_span - 1) // block_span
